...
wire cs_udac;
assign cs_udac= (ps_sys_addr[15:14]== `ADDR_COMMON)
...
wire cs_uadc;
assign cs_uadc= (ps_sys_addr[15:14]== `ADDR_COMMON)
                &(ps_sys_addr[13:12]== `ADDR_ADC_REGS)
                 &(ps_sys_cs);                      

u_adc u_adc_(.i_ps_clk(ps_sys_clk),.i_clk(clk80),.i_clr(clr)
            ,.i_delay_clk(fclk[1])
  			,.i_cs(cs_uadc),.i_we(ps_sys_wen),.i_addr(ps_sys_addr)
			,.i_data(ps_sys_wdata[15:0]),.o_data(uadc_odata)
             ,.i_adc_clk_p(i_adc_clko_p),.i_adc_clk_n(i_adc_clko_m)
             ,.i_data_p(i_adc_dat_p),.i_data_n(i_adc_dat_m)
