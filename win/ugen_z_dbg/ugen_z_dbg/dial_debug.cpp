...

{
	...
	ui->comboBox_rej->addItem("UDB",UDB_REJ);
	ui->comboBox_rej->addItem("UPOW",UPOW_REJ);
}
...
if(ui->comboBox_rej->currentData()==XIL_REJ)
	emit req_rd_xil(&alt_dat_req);
else if(ui->comboBox_rej->currentData()==STM_REJ) 
	emit req_rd_stm(&alt_dat_req);
else if(ui->comboBox_rej->currentData()==MSP_REJ) 
	{
	tmp=ui->lineEdit_addr->text().toLatin1();
	emit req_rd_msp(tmp);
	}
else if(ui->comboBox_rej->currentData()==UDB_REJ) 
	{
	tmp=ui->lineEdit_addr->text().toLatin1();
	emit req_rd_udb(&alt_dat_req);
	}
else if(ui->comboBox_rej->currentData()==UPOW_REJ) 
	{
	tmp=ui->lineEdit_addr->text().toLatin1();
	emit req_rd_upow(&alt_dat_req);
	}
}
...
///		tmpb[ui->lineEdit_addr->text().size()] = 0;
	...
	emit req_wr_udb(&alt_dat_req);
else if(ui->comboBox_rej->currentData()==UPOW_REJ)
	emit req_wr_upow(&alt_dat_req);
}
