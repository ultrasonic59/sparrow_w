...
///=================================
int puts_xil_dat(alt_dat_req_t *alt_req)
{
	uint8_t *ptbuff;
	uint32_t *ptbuff32;
////	ptbuff=(uint32_t *)ugen_regs;
	ptbuff=(uint8_t *)fpga_regs;
	ptbuff32=(uint32_t*)(ptbuff+alt_req->addr);
///	*((uint32_t*)(ptbuff+alt_req->addr))=alt_req->data[0];
	*ptbuff32=alt_req->data[0];
///	ptbuff[alt_req->addr]=alt_req->data[0];
	fprintf(stderr,"\nputs_xil_dat[%x:%x:%x] ",alt_req->addr,alt_req->data[0],(uint32_t)ptbuff32);
	return 0;
}
///=================================
int gets_xil_dat(alt_dat_req_t *alt_req)
{
	uint8_t *ptbuff;
	ptbuff=(uint8_t *)fpga_regs;
	memcpy(alt_req,&g_alt_dat,sizeof(alt_dat_req_t));
	alt_req->data[0]=*((uint32_t*)(ptbuff+alt_req->addr));
	fprintf(stderr,"\ngets_xil_dat[%x:%x] ",alt_req->addr,alt_req->data[0]);
	return 0;
}
